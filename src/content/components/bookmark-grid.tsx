...
function MediaPreview({media}: {media: NonNullable<BookmarkPreview['media']>[number]}) {
  const mediaClassName = 'xbo:block xbo:h-full xbo:w-full xbo:object-contain'
  if (media.type === 'video') return <div className="xbo:mb-4 xbo:aspect-video xbo:overflow-hidden xbo:rounded-lg xbo:bg-neutral-950"><video className={mediaClassName} controls preload="none" poster={media.previewUrl}><source src={media.url} /></video></div>
  return <div className="xbo:mb-4 xbo:aspect-video xbo:overflow-hidden xbo:rounded-lg xbo:bg-neutral-950"><img className={mediaClassName} src={media.url} alt="" loading="lazy" /></div>
}
