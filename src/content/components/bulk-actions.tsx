    ...
    <div className="xbo:mx-6 xbo:mt-4 xbo:flex xbo:flex-wrap xbo:items-center xbo:gap-3 xbo:rounded-lg xbo:border xbo:border-white/10 xbo:bg-neutral-900 xbo:px-4 xbo:py-3">
      ...
      {selectedCount > 0 && (
        <>
          ...
          <button className="xbo:cursor-pointer xbo:rounded-full xbo:border xbo:border-white/35 xbo:bg-neutral-800 xbo:px-3 xbo:py-1.5 xbo:text-sm xbo:text-white xbo:hover:bg-white xbo:hover:text-black xbo:disabled:cursor-wait xbo:disabled:opacity-50" type="button" onClick={onAddToFolder} disabled={removing || updatingFolders}>Add to folder</button>
          <button className="xbo:cursor-pointer xbo:rounded-full xbo:border xbo:border-white/35 xbo:bg-neutral-800 xbo:px-3 xbo:py-1.5 xbo:text-sm xbo:text-white xbo:hover:bg-white xbo:hover:text-black xbo:disabled:cursor-wait xbo:disabled:opacity-50" type="button" onClick={onRemoveFromFolder} disabled={removing || updatingFolders}>Remove from folder</button>
