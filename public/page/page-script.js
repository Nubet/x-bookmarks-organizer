...
(() => {
    ...
    const BOOKMARKS_ENDPOINT = 'https://x.com/i/api/graphql/QUjXply7fA7fk05FRyajEg/Bookmarks';
    ...
    function captureTransactionId(input, init) {
        const url = getRequestUrl(input);
        if (!url.includes('/i/api/graphql/'))
            return;
        const transactionId = readHeader(init?.headers ?? (input instanceof Request ? input.headers : undefined), 'x-client-transaction-id');
        if (!transactionId)
            return;
    ...
    async function fetchBookmarks(cursor) {
        ...
        if (!csrfToken)
            throw new Error('X CSRF token not found');
        const variables = {
            count: 100,
        ...
        if (!response.ok)
            throw new Error(`X bookmark sync failed: HTTP ${response.status}`);
        return parseBookmarkPage(await response.json(), cursor);
    }
    function parseBookmarkPage(data, cursor) {
        ...
        return {
            bookmarks,
            nextCursor: typeof candidateCursor === 'string' && candidateCursor !== cursor
                ? candidateCursor
                : null,
        };
    }
    function readEntries(data) {
        const value = data;
        const entries = value.data?.bookmark_timeline_v2?.timeline?.instructions?.flatMap((instruction) => instruction.entries ?? []);
        return (entries ?? []).filter(isTimelineEntry);
    }
    function isTimelineEntry(value) {
    ...
    window.addEventListener('message', (event) => {
        ...
        void (async () => {
            try {
                const data = operation === 'GET_TRANSACTION_ID'
                ...
                window.postMessage({ type: RESPONSE_TYPE, requestId, ok: true, data }, window.location.origin);
            }
            catch (error) {
                window.postMessage({
                    type: RESPONSE_TYPE,
